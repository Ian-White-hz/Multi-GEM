...
class MPC:
    ...
    def robot_cost(self, control_inputs_r):
        ...
        self.optimize_h()
        return self.dist_to_human * 100 - self.speed_of_robot
    ...
    def human_cost(self, control_inputs_h):
        ...
        for idx in range(self.horizon):
            ...
            self.speed_of_robot += robot_car.velocity.x
            dist_to_robot += max([0, 30 - (robot_car.center - human_car.center).norm()])
            speed_of_human += human_car.velocity.y
            robot_car.set_control(0, self.robot_actions[idx])
            human_car.set_control(0, self.human_actions[idx])
