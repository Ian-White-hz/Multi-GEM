...
def run_model():
    ...
    # init_time_diff = (init_time - start_time).to_sec()
    # rospy.loginfo("Initialization time: " + str(init_time_diff))
    ...
    while not rospy.is_shutdown() :
        ...
        # rospy.loginfo("Loop time: " + str((loop_end_time - cur_time).to_sec()))
        ...
        # if (currState_e2.pose.position.x < currState_e4.pose.position.x) and start_merging == False:
        #     start_merging = True
            
        # if start_merging and not end_merging:
        #     target_y_e4 = -17.5
        #     e4_max_speed = 5
        #     ttc_threshold = 8
        #     rospy.loginfo("******MERGING******")
        #     # Check if the vehicles finish merging
        #     if (currState_e4.pose.position.x > 0.5) and (currState_e4.pose.position.y < -17.5):
        #         rospy.loginfo("******FINISHED MERGING******")
        #         end_merging = True
        #         start_merging = False
        #         e4_max_speed = 5
        #         target_y_e4 = -17.5
        #     # Check for collisions
        #     if ttc_mid_mid < ttc_threshold and ttc_mid_mid != -1:
        #         rospy.loginfo("******ttc_mid_mid COLLISION******")
        #         rospy.loginfo(f"Time to collision: {ttc_mid_mid}")  
        #         end_merging = True  
        #     if ttc_front_mid < ttc_threshold and ttc_front_mid != -1:
        #         rospy.loginfo("******ttc_front_mid COLLISION******")
        #         rospy.loginfo(f"Time to collision: {ttc_front_mid}")
        #         end_merging = True
        #     if ttc_rear_mid < ttc_threshold and ttc_rear_mid != -1:
        #         rospy.loginfo("******ttc_rear_mid COLLISION******")
        #         rospy.loginfo(f"Time to collision: {ttc_rear_mid}")
        #         end_merging = True
        #     if ttc_mid_front < ttc_threshold and ttc_mid_front != -1:
        #         rospy.loginfo("******ttc_mid_front COLLISION******")
        #         rospy.loginfo(f"Time to collision: {ttc_mid_front}")
        #         end_merging = True
        #     if ttc_mid_rear < ttc_threshold and ttc_mid_rear != -1:
        #         rospy.loginfo("******ttc_mid_rear COLLISION******")
        #         rospy.loginfo(f"Time to collision: {ttc_mid_rear}")
        #         end_merging = True
        #     if ttc_front_front < ttc_threshold and ttc_front_front != -1:
        #         rospy.loginfo("******ttc_front_front COLLISION******")
        #         rospy.loginfo(f"Time to collision: {ttc_front_front}")
        #         end_merging = True
        #     if ttc_front_rear < ttc_threshold and ttc_front_rear != -1:
        #         rospy.loginfo("******ttc_front_rear COLLISION******")
        #         rospy.loginfo(f"Time to collision: {ttc_front_rear}")
        #         end_merging = True
        #     if ttc_rear_front < ttc_threshold and ttc_rear_front != -1:   
        #         rospy.loginfo("******ttc_rear_front COLLISION******")
        #         rospy.loginfo(f"Time to collision: {ttc_rear_front}")
        #         end_merging = True
        #     if ttc_rear_rear < ttc_threshold and ttc_rear_rear != -1:
        ...
        # if (cur_time - prev_wp_time).to_sec() > 8:  #might change time based on dist btwn waypoints
