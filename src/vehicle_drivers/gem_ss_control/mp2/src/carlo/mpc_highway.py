import numpy as np
from scipy.optimize import minimize, LinearConstraint
import copy


class MPC:

    def __init__(self, horizon, max_iter):
        self.horizon = horizon
        self.max_iter = max_iter
        self.world = None
        self.agents = None
        self.robot_actions = None
        self.human_actions = None
        self.control_inputs_r = np.array([0.0]*self.horizon*2)
        self.control_inputs_h = np.array([0.0]*self.horizon*2)
        self.robot_const = LinearConstraint(np.eye(self.horizon*2), -20, 20)
        self.human_const = LinearConstraint(np.eye(self.horizon*2), -20, 20)
        self.dist_to_human = None
        self.block_human = None
        self.heading = None

    def update_world(self, world):
        self.world = world
        self.reset()

    def reset(self):
        self.agents = copy.deepcopy(self.world.dynamic_agents)

    def optimize_h(self):
        res = minimize(self.human_cost, self.control_inputs_h, method='SLSQP', constraints=self.human_const, options={'eps': 1e-3, 'maxiter': self.max_iter})

    def optimize_r(self):
        res = minimize(self.robot_cost, self.control_inputs_r, method='SLSQP', constraints=self.robot_const, options={'eps': 1e-3, 'maxiter': self.max_iter})
        # print("robot cost: ", res.fun)
    def robot_cost(self, control_inputs_r):
        self.robot_actions = control_inputs_r.reshape(self.horizon, 2)
        self.optimize_h()
        robot_car = self.agents[0]
        
        # Penalize current low speed
        # print("speed", robot_car.velocity)
        speed_penalty = max(0, 4 - robot_car.velocity.x)**2

        
        # print("robot car heading: ", robot_car.heading)

        # print(self.dist_to_human)
        # print(self.block_human)
        # print(self.heading)
        # print(speed_penalty)
         
        return self.dist_to_human * 400 + self.block_human * 50 + self.heading * 320 #+ speed_penalty * 500
        # return self.dist_to_human * 100 + self.block_human * 25 + self.heading * 150

    def human_cost(self, control_inputs_h):
        self.human_actions = control_inputs_h.reshape(self.horizon, 2)
        robot_car = self.agents[0]
        human_car = self.agents[1]

        # print("robot car center:", robot_car.center.x, robot_car.center.y, robot_car.heading)  
        # print("human car center:", human_car.center.x, human_car.center.y, human_car.heading)

        self.dist_to_human = 0
        self.block_human = 0
        self.heading = 0
        self.human_heading = 0
        dist_to_robot = 0
        speed_of_human = 0
        dis_to_center = 0
        # print("human car center", human_car.center.x, human_car.center.y)
        for idx in range(self.horizon):
            self.dist_to_human += max([0, 50 - (robot_car.center - human_car.center).norm()])
            self.block_human += (human_car.center.y - robot_car.center.y)**2
            
            
            # self.heading += (np.mod(np.abs(robot_car.heading - 2*np.pi), 2*np.pi))**2
            self.heading += robot_car.heading**2
            self.human_heading += human_car.heading**2
            dist_to_robot += max([0, 30 - (robot_car.center - human_car.center).norm()])
            dis_to_center += (0.4 * (human_car.center.y -2))**4
            speed_of_human += human_car.velocity.x**2


            robot_car.set_control(self.robot_actions[idx, 0], self.robot_actions[idx, 1])
            human_car.set_control(self.human_actions[idx, 0], self.human_actions[idx, 1])
            robot_car.tick(self.world.dt)
            human_car.tick(self.world.dt)
        self.reset()
        return dist_to_robot - speed_of_human + self.human_heading * 5 + 0.05 * dis_to_center
